    ...
    public void onMessage(Message message) {
        try {
            ...
            for (Iterator<SincerelyCreateResponseSendTo> it = scr.getSent_to().iterator(); it.hasNext();) {
                ...
                File f = File.createTempFile("test-" + scrst.getPrintId(), "pdf");
                respdf.getEntity().writeTo(new FileOutputStream(f));
                Runtime.getRuntime().exec("open " + f.getAbsolutePath());
            }
            
            Logger.getAnonymousLogger().info(res.getStatusLine().toString());
